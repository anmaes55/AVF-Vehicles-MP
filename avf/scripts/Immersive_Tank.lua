...
function kill_tank(max_intensity)
	...
	-- if(not IsHandleValid(tank.cook_off_loc)) then 
		...
		com= VecAdd(com,Vec(0,0.65,0))
		-- DebugWatch("com",com)
	tank.cook_off_origin = Transform(com)
...
function check_ammo_stability()
	if(not tank.cooking_off and #tank.ammo_racks>0) then 
		...
		if(tank.cooking_off) then 
			...
			SetTag(tank.id, "avf_vehicle_disabled")

			SetTag(tank.id, "AVF_status","disabled")
			if(IsHandleValid(base_vehicle)) then 
				SetTag(tank.base_vehicle, "AVF_status","disabled")
				SetTag(tank.base_vehicle, "avf_vehicle_disabled")
...
function cook_off()
	...
	-- DebugWatch("cook offorigin",tank.cook_off_origin)
